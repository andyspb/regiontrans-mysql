...
{if l=2 then
begin
  q:=sql.select('Send','SumCount,Client_Ident,PayType_Ident','Client_Ident='+
...
{  begin
  ...
  q:=sql.select('`Order`','SumNDS,Client_Ident','Client_Ident='+IntToStr(Id),'');
...
{        q:=sql.select('PaySheet','Sum,Client_Ident','Client_Ident='+IntToStr(Id),'');
...
S2:=StrTo00(S2);
S1:=StrTo00(S1);
       if  (S1=S2)  then CreditType:='0.00'
           else  CreditType:=FloatToStr(Sum1-Sum);
...
if id<>0 then
begin
q:=sql.select('Send','SumCount,Client_Ident','Client_Ident='+IntToStr(Id),'');
...
S2:=StrTo00(S2);
S1:=StrTo00(S1);
if  (S1=s2)  then Credit:='0.00'
    else  Credit:=FloatToStr(Sum1-Sum);
...
if q.FieldByName('Saldo').asFloat<0 then    {-}
  ...
  else  Sum1:=Sum1+q.FieldByName('Saldo').asFloat;    {+}  
q.Free;
//---------------------
S1:='';
S2:='';
...
if  (S1=S2)  then CreditDate2007:='0.00'
    else  CreditDate2007:=FloatToStr(Sum1-Sum);

end else CreditDate2007:='0.00';
...
if  (S1=S2)  then CreditDate:='0.00'
    else  CreditDate:=FloatToStr(Sum1-Sum);
...
1:begin
  if j=1 then s:='';
  if j=2 then s:=' тысяча';
  if j=3 then s:=' миллион';
  if j=4 then s:=' миллиард';
  end;
2..4:begin
  if j=1 then s:='';
...
0,5..9:begin
  if j=1 then s:='';
  if j=2 then s:=' тысяч';
  if j=3 then s:=' миллионов';
  if j=4 then s:=' миллиардов';
  end;
end;
String0:=str+s;
end;
...
if day<>0 then Da:='"'+IntToStr(Day)+'"'
 else Da:='';
if Year<>0 then Y:=IntToStr(Year)+' г.'
  else Y:='';
DataDMstrY:=Da+' '+M+' '+Y;
end;
